...
class SyncWorker(
    ...
    override fun doWork(): Result {
        val db = CosmicMoneySQLiteOpenHelper.getInstance(context)
        ...
        for (proj in db.projects) {
            ...
            val syncTask = db.cosmicMoneyServerSyncHelper.scheduleSync(false, proj.id)
            // await task
    ...
    private fun showNotification(context: Context, db: CosmicMoneySQLiteOpenHelper) {
        CosmicMoneyServerSyncHelper.createNotificationChannels(context, db)

        val channelId = CosmicMoneyServerSyncHelper.MAIN_CHANNEL_ID.toString()
