...
import net.bobotig.cosmicmoney.android.activity.QrCodeScannerActivity;
import net.bobotig.cosmicmoney.databinding.ActivityNewProjectBinding;
...
import net.bobotig.cosmicmoney.theme.ThemedProgressDialogBuilder;
...
public class NewProjectFragment extends ThemedFragment {
    ...
    @Nullable
    protected CosmicMoneySQLiteOpenHelper db;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        boolean darkTheme = CosmicMoney.isDarkTheme(getContext());
    ...
    private boolean isFormValid() {
        ...
        boolean isIhmInvitationLink = type.equals(ProjectType.ILOVEMONEY)
        ...
        if (!type.equals(ProjectType.LOCAL)) {
            ...
            if (todoCreate) {
                if (type.equals(ProjectType.ILOVEMONEY) && projectPassword.equals("")) {
    ...
    private void showHideInputFields(boolean setDefaultUrl) {
        ...
        if (type.equals(ProjectType.LOCAL)) {
        ...
        } else if (type.equals(ProjectType.ILOVEMONEY)) {
            ...
            boolean isNCC = CosmicMoneyServerSyncHelper.isNextcloudAccountConfigured(getContext());
    ...
    public void onAttach(@NonNull Context context) {
        ...
        db = CosmicMoneySQLiteOpenHelper.getInstance(context);
    }
    ...
    private void onPressOk() {
        ...
        if (isValidUrl(url) && todoCreate && ProjectType.COSPEND.equals(type) &&
                db.getCosmicMoneyServerSyncHelper().canCreateAuthenticatedProject(fakeProj)) {
    ...
    private void createProject() {
        ...
        if (!ProjectType.LOCAL.equals(type)) {
            if (!isIhmInvitationLink && !isCospendScheme) {
                ...
                if (ProjectType.ILOVEMONEY.equals(type)) {
        ...
        if (!todoCreate || ProjectType.LOCAL.equals(type)) {
            ...
            if (!db.getCosmicMoneyServerSyncHelper().createRemoteProject(getRemoteId(), getName(),
    ...
    protected void saveRemoteProject(@Nullable ICallback callback, boolean ignorePassword) {
        ...
        if (!db.getCosmicMoneyServerSyncHelper().getRemoteProjectInfo(newProject, getRemoteInfoCallBack)) {
    ...
    private DBProject getProjectFromFields(boolean ignorePassword) {
        ...
        if (type.equals(ProjectType.ILOVEMONEY) && isInvitationLink(url)) {
            password = "";
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        if (defaultTypeId != null) {
            if (defaultTypeId.equals(ProjectType.COSPEND.getId())) {
                ...
                whereIhm.setChecked(false);
                whereCospend.setChecked(true);
            } else if (defaultTypeId.equals(ProjectType.ILOVEMONEY.getId())) {
                whereLocal.setChecked(false);
                whereIhm.setChecked(true);
                whereCospend.setChecked(false);
        ...
        if (ProjectType.ILOVEMONEY.getId().equals(defaultTypeId)
                && isFormValid()
